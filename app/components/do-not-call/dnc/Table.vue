...
const selectedDncForDelete = ref<{
  number: number | null
}>({
  number: null,
})
...
async function handleDelete() {
  ...
  try {
    const res = await useApi().post('/delete-dnc', {
      number: selectedDncForDelete.value.number,
    })
...
const columns = [
  ...
  columnHelper.accessor('number', {
    header: ({ column }) => {
      return h('div', { class: 'text-center' }, h(Button, { class: 'text-center text-sm font-normal', variant: 'ghost', onClick: () => column.toggleSorting(column.getIsSorted() === 'asc') }, () => ['Number', h(ChevronsUpDown, { class: 'ml-2 h-4 w-4' })]))
    },
    cell: ({ row }) => {
