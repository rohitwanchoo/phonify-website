  ...
  columnHelper.accessor('status', {
    header: ({ column }) =>
      h('div', { class: 'text-center' }, h(Button, {
        ...
        class: 'text-sm font-normal',
        onClick: () => column.toggleSorting(column.getIsSorted() === 'asc'),
      }, () => ['Delivery Status', h(ChevronsUpDown, { class: 'ml-2 h-4 w-4' })])),
    cell: ({ row }) =>
      h('div', { class: 'flex justify-center' }, h('span', {
        class: cn(
          'px-3 py-1 w-[80px] text-center text-xs rounded-full font-medium',
          row.original.status === 'Success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white',
        ),
      }, row.original.status)),
  }),
